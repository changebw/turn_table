...
bool comparePot(int pin, int score) {
  int originalState = readPot(pin);
  int n = 0;
  int threshold = 3000 - 30*score;
  bool check;

  while (true) {
    if (abs(readPot(pin) - originalState) > 50) {
      return true;
    } 
...
bool compareButton(int pin, int score) {
  int n = 0;
  int threshold = 3000 - 30*score;
  bool check;

  while (true) {
    if (readButton(pin) == HIGH) {
      return true;
    } 
    else if (n * 20 > threshold) {
      check = false;
...
void game() {
  ...
  while (true) {
    int input = random(0,3);
    display.clearDisplay();
    printtext("Input: ", 1, 0);
    printtext(input, 50, 0);
    printtext("Score: ", 1, 25);
    printtext(score, 50, 25);

    switch (input) {
      case 0:
        //playSound("turntable.wav");
        cont = compareButton(buttonPin, score);
        break;
      case 1:
        //playSound("knob.wav");
        cont = comparePot(potPin, score);
        break;
      case 2:
        //playSound("slider.wav");
        cont = comparePot(slidePin, score);
        break;
    }
  ...
  printtext("Final Score: ", 0, 10);
  printtext(score, 100, 10);
  printtext("Use Turntable to Play Again", 0, 25);
}